...
const hasUsableRecommendations = (doc) => {
  return Array.isArray(doc?.recommendedJobs) && doc.recommendedJobs.length > 0;
};
...
export const generateRecommendations = async (candidateId) => {
  ...
  const { candidate, hasParsedResume } = await loadCandidateProfile(candidateId);
  const { jobs, appliedJobIds } = await fetchEligibleJobs(candidateId, dismissedJobIds);

  if (!hasParsedResume || !jobs.length || !(candidate.skills?.length || candidate.embeddings?.length)) {
...
export const getOrGenerateRecommendations = async (candidateId, { forceRefresh = false } = {}) => {
  ...
  if (!forceRefresh && current && !isStale(current) && hasUsableRecommendations(current)) {
