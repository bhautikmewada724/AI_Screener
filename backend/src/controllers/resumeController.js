...
export const uploadResume = async (req, res, next) => {
  try {
    ...
    try {
      ...
      const message = error?.message || 'AI resume parsing failed.';
      resume.status = 'failed';
      resume.parsedData = { error: message };
    ...
    // Invalidate cached recommendations so they regenerate from the latest parsed resume
    await Recommendation.deleteOne({ candidateId: req.user.id });
...
export const patchParsedData = async (req, res, next) => {
  try {
    ...
    // Invalidate cached recommendations so edits to skills/location/summary are reflected
    await Recommendation.deleteOne({ candidateId: req.user.id });
