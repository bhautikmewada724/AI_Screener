...
def _detect_education(text: str) -> List[str]:
  """Detect education requirements from JD text."""
  edu_patterns = [
    r"(?:bachelor'?s?|b\.?s\.?|b\.?a\.?|b\.?tech|bca|bsc)\s+(?:degree|in\s+\w+)?",
    r"(?:master'?s?|m\.?s\.?|m\.?a\.?|m\.?tech|mca|mba|msc)\s+(?:degree|in\s+\w+)?",
    r"(?:ph\.?d\.?|doctorate)\s*(?:degree|in\s+\w+)?",
    r"(?:degree|diploma)\s+in\s+\w+",
  ]
  found = []
...
def build_requirements(
  ...
  edu_requirements = _detect_education(job_text)
  for req in edu_requirements:
    terms = normalize_token(req).split()
    requirements.append(
      JDRequirement(
        id=_stable_id('education', terms),
        type='education',
        rawText=req,
        normalizedTerms=terms,
        weight=0.6,
        isRequired=False,
        explicitlyStated=True,
        evidenceRule='Matching degree mentioned in Education section.'
      )
    )
