...
const CandidateResumesPage = () => {
  ...
  const resumesQuery = useQuery({
    ...
    refetchInterval: (data) => {
      ...
      return data.some((resume) => resume.status === 'processing') ? 4000 : false;
    }
  });
...
const ResumeCard = ({ resume, token }: { resume: ResumePayload; token?: string }) => {
  ...
  const handleSave = () => {
    ...
    const cleanedSkills = draftSkills
      .map((skill) => skill.trim())
      .filter(Boolean)
      .filter((skill, index, list) => list.findIndex((item) => item.toLowerCase() === skill.toLowerCase()) === index);
    if (cleanedSkills.length) payload.skills = cleanedSkills;
    if (typeof draftLocation === 'string' && draftLocation.trim()) payload.location = draftLocation.trim();
    if (draftYears !== '') {
    ...
    >
      {resume.status === 'failed' && (
        <div className="rounded-xl border border-rose-100 bg-rose-50 px-3 py-2 text-sm text-rose-700">
          ...
          {resume.parsedData?.error ? `${resume.parsedData.error}.` : ''} Please retry with a different file.
        </div>
      )}
      <div className="rounded-2xl border border-dashed border-slate-100 bg-slate-50/60 p-4">
        {resume.status === 'parsed' ? (
          ...
          <div className="flex items-start gap-3 text-sm text-brand-ash">
            <span className="mt-1 inline-block h-3 w-3 animate-spin rounded-full border-2 border-slate-300 border-t-slate-700" />
            <div>
              <p className="font-semibold text-brand-navy">Parsing resume…</p>
              <p>We are extracting and analyzing your resume. This usually takes a few seconds.</p>
            </div>
          </div>
        ) : (
          <p className="text-sm text-brand-ash">Upload a new file to retry parsing.</p>
        )}
      </div>
    </SectionCard>
  );
